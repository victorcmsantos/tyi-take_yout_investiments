...
def graficos():
    selected_portfolio_ids = _selected_portfolio_ids()
    benchmark_range = (request.args.get("bench_range") or "12m").strip().lower()
    benchmark_scope = (request.args.get("bench_scope") or "all").strip().lower()
    ...
    annual_selected_categories = [item for item in raw_annual_categories if item in valid_annual_categories]
    if not annual_selected_categories:
        annual_selected_categories = ["br", "us", "fii", "cripto", "fixa"]
...
def setores():
    selected_portfolio_ids = _selected_portfolio_ids()
    return redirect(url_for("main.home", portfolio_id=selected_portfolio_ids))
